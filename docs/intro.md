...
- **No compilation step for rules** - rules are plain JSON files with no compile-time dependencies, so any text editor, version control system, or LLM can read, generate, and diff them without a build step.
...
- **Composable and parameterised** - rules support variables, nested conditional logic and easy-to-use Fabric operators (REST API GET and Scanner API calls, DAX queries, SQL queries and OneLake reads).
...
- providing pre-flight guardrail hints to guide the agent's actions when generating Fabric artifacts or amending a Fabric tenant's configuration
...
- optionally logging validation results as JSON to Fabric OneLake for post-hoc reporting or alerting
